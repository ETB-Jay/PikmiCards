...
const Tags = memo(({ tags }: TagsProps) => {
    ...
    const renderTag = useCallback((tag: string, index: number) => (
        <span key={index} className={"flex flex-row items-center justify-center text-nowrap bg-green-950 border border-green-400/30 rounded-2xl px-2 py-0.5 text-silver-100 text-xs"}>
            {tag}
        </span>
    ), []);
