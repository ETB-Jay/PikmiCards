...
const SelectField = memo(({ id, name, icon, options, value, onChange }: SelectFieldProps) => (
  <div className="relative flex h-full w-full min-w-fit flex-col items-center justify-center">
    ...
    <select
      ...
      className={cn(
        ...
        "pl-10 transition-all placeholder:text-stone-600 focus:ring-2 focus:outline-none",
        "border-green-smoke-200 focus:ring-green-smoke-400 cursor-pointer bg-white/80"
      )}
    >
      {options.map((option) => (
